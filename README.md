...
One role-scoped mobile UI for supervisors (plant-wide, cross-shift) and leads (shift-level, 3 rebar cells). Frames 1–7 are situational-awareness + reroute; 8–10 are the reusable `hpt-approvals` queue (approver side); 11–14 are the 4 self-service surfaces each in a Supervisor and an Office view (both SSO-gated, internal-only); 15–16 are **held** placeholders pending open questions. Design note in the private repo at `docs/spec/supervisor-lead-mobile-design-2026-07-24.md`.
